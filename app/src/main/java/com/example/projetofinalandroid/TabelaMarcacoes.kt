...
class TabelaMarcacoes(db: SQLiteDatabase) : BaseColumns  {
    ...
    fun cria() {
        db.execSQL("CREATE TABLE $NOME_TABELA (${BaseColumns._ID} INTEGER PRIMARY KEY AUTOINCREMENT, $CAMPO_DATA_DOSE INTEGER NOT NULL,$CAMPO_ID_VACINA INTEGER NOT NULL,$CAMPO_ID_PESSOA INTEGER NOT NULL, FOREIGN KEY ($CAMPO_ID_VACINA) REFERENCES ${TabelaVacina.NOME_TABELA}, FOREIGN KEY ($CAMPO_ID_PESSOA) REFERENCES ${TabelaPessoas.NOME_TABELA} )")
